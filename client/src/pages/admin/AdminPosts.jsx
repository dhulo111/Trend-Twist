...
import { Search, Image as ImageIcon, Trash2, ShieldAlert, ChevronLeft, ChevronRight } from 'lucide-react';
...
const AdminPosts = () => {
  ...
  const fetchPosts = async (pageNum = 1, search = '') => {
    try {
      ...
      const res = await axios.get(`${API_URL}/admin/posts/?page=${pageNum}&search=${encodeURIComponent(search)}`, {
  ...
  const deletePost = async (postId) => {
    if (!window.confirm("Are you sure you want to delete this post maliciously violating terms of service?")) return;
    try {
      const token = localStorage.getItem('access_token');
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Review posts, media, and handle reported content.</p>
        </div>
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto">
          <div className="relative flex-1 sm:w-64">
            ...
            <input
              type="text"
              placeholder="Search ID, content, or author..."
              value={searchTerm}
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl overflow-hidden shadow-sm border border-gray-200 dark:border-gray-700 flex flex-col min-h-[500px]">
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
              {loading ? (
                <tr>
                  <td colSpan="5" className="p-16 text-center text-gray-500">
                    <div className="flex flex-col items-center justify-center">
                      ...
                      <p className="mt-4 font-medium">Loading posts...</p>
                    </div>
                  </td>
                </tr>
              ) : posts.length > 0 ? (
                  ...
                  <tr key={post.id} className="hover:bg-gray-50/50 dark:hover:bg-gray-700/20 transition-colors">
                    ...
                    <td className="p-4">
                      <div className="flex items-center gap-2">
                        ...
                        {post.is_private && <ShieldAlert size={14} className="text-yellow-500" title="Private Post" />}
                      </div>
                      <div className="text-xs text-gray-500 font-mono mt-1">ID: {post.id}</div>
                    </td>
                    <td className="p-4">
                ...
                <tr>
                  <td colSpan="5" className="p-16 text-center text-gray-500">
                    <ImageIcon size={48} className="mx-auto mb-4 text-gray-300 dark:text-gray-600" />
        ...
        <div className="px-6 py-4 border-t border-gray-200 dark:border-gray-700 flex items-center justify-between text-sm text-gray-500 bg-gray-50/30 dark:bg-gray-900/30">
          <span>Showing page <span className="font-semibold text-gray-900 dark:text-gray-100">{page}</span> of <span className="font-semibold text-gray-900 dark:text-gray-100">{totalPages}</span> ({totalPosts} total posts)</span>
