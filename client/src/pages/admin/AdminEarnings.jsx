...
import {
  ...
  FaWallet, FaHistory, FaUserCircle, FaCreditCard, FaUniversity
} from 'react-icons/fa';
    ...
    <tr className="border-t border-gray-800 hover:bg-gray-800/20 transition-colors">
      <td className="px-5 py-4">
        <div className="flex items-center gap-2">
           ...
           <div>
              <p className="font-bold text-white text-sm">@{item.creator_username || 'Creator'}</p>
              <p className="text-[10px] text-gray-500">{new Date(item.created_at).toLocaleString()}</p>
           </div>
        </div>
      </td>
      <td className="px-5 py-4 font-black text-white">₹{parseFloat(item.amount).toFixed(2)}</td>
      <td className="px-5 py-4">
...
const CreatorDetail = ({ creator, onBack }) => {
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-left-4 duration-300">
      ...
      {/* Transaction History */}
      <div>
        <h4 className="font-bold text-white mb-3 flex items-center gap-2"><FaChartBar className="text-purple-400" /> Subscription Credits</h4>
            ...
            <tbody className="divide-y divide-gray-800/50">
              ...
              {detail?.earnings?.map(e => (
                <tr key={e.id} className="hover:bg-gray-800/30">
                  ...
                  <td className="px-4 py-3 font-semibold text-white">@{e.subscriber}</td>
      ...
      {/* Withdrawal History */}
      <div>
        <h4 className="font-bold text-white mb-3 flex items-center gap-2"><FaHistory className="text-blue-400" /> Withdrawal History</h4>
        <div className="overflow-x-auto rounded-xl border border-gray-800 bg-gray-900/40">
          <table className="w-full text-left text-sm text-gray-300">
            <thead className="bg-gray-800 text-gray-500 text-xs uppercase">
              <tr>
            ...
            <tbody className="divide-y divide-gray-800/50">
              ...
              {detail?.withdrawals?.map(w => (
                <tr key={w.id} className="hover:bg-gray-800/30">
                  ...
                  <td className="px-4 py-3 text-white font-black">₹{parseFloat(w.amount).toFixed(2)}</td>
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex items-center justify-between">
         ...
         <div className="text-right hidden sm:block">
            ...
            <p className="text-xs text-gray-400">Total System Volume: ₹{grossTotal.toLocaleString()}</p>
         </div>
      </div>
      ...
      {/* Tab Controls */}
      <div className="flex items-center gap-2 p-1 bg-gray-900 border border-gray-800 rounded-xl w-fit">
         <button 
            onClick={() => setActiveTab('withdrawals')}
            className={`px-6 py-2 rounded-lg text-xs font-bold transition-all ${activeTab === 'withdrawals' ? 'bg-purple-600 text-white shadow-lg' : 'text-gray-500 hover:text-white'}`}
         >
         ...
         <button 
            ...
            className={`px-6 py-2 rounded-lg text-xs font-bold transition-all ${activeTab === 'creators' ? 'bg-purple-600 text-white shadow-lg' : 'text-gray-500 hover:text-white'}`}
         >
            Creator Master List
         </button>
      </div>
           ...
           <div className="bg-gray-900 rounded-2xl border border-gray-800 overflow-hidden shadow-2xl">
              <div className="overflow-x-auto">
                 <table className="w-full text-left text-sm">
                    <thead className="bg-gray-800/80 text-gray-500 text-xs uppercase font-black">
                       <tr>
           ...
           <div className="flex items-center justify-between">
              ...
              <div className="relative w-64">
                ...
                <input
                  ...
                  className="w-full pl-8 pr-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-white text-xs focus:outline-none focus:border-purple-500"
                />
              </div>
           </div>
