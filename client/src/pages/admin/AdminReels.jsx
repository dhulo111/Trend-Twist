...
import { Search, Video, Trash2, ChevronLeft, ChevronRight } from 'lucide-react';
...
const AdminReels = () => {
  ...
  const fetchReels = async (pageNum = 1, search = '') => {
    try {
      setLoading(true);
      const token = localStorage.getItem('access_token');
      const res = await axios.get(`${API_URL}/admin/reels/?page=${pageNum}&search=${encodeURIComponent(search)}`, {
        headers: { Authorization: `Bearer ${token}` }
      });

      setReels(res.data.results);
      setTotalReels(res.data.count);
  ...
  const deleteReel = async (reelId) => {
    ...
    try {
      const token = localStorage.getItem('access_token');
      await axios.delete(`${API_URL}/admin/reels/${reelId}/`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      // Refresh current page
      fetchReels(page, searchTerm);
    } catch (err) {
      alert("Failed to delete reel.");
    }
  };

  const totalPages = Math.ceil(totalReels / PAGE_SIZE) || 1;

  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Reels Moderation</h2>
          <p className="text-sm text-gray-500 mt-1">Review short video content across the platform.</p>
        </div>
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto">
          <div className="relative flex-1 sm:w-64">
            ...
            <input
              type="text"
              placeholder="Search ID, caption, or author..."
              value={searchTerm}
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl overflow-hidden shadow-sm border border-gray-200 dark:border-gray-700 flex flex-col min-h-[500px]">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50/50 dark:bg-gray-900/50 text-gray-500 text-sm uppercase tracking-wider border-b border-gray-200 dark:border-gray-700">
                ...
                <th className="p-4 font-medium w-1/2">Caption</th>
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
              {loading ? (
                <tr>
                  <td colSpan="5" className="p-16 text-center text-gray-500">
                    <div className="flex flex-col items-center justify-center">
                      ...
                      <p className="mt-4 font-medium">Loading reels...</p>
                    </div>
                  </td>
                </tr>
              ) : reels.length > 0 ? (
                  ...
                  <tr key={reel.id} className="hover:bg-gray-50/50 dark:hover:bg-gray-700/20 transition-colors">
                    ...
                    <td className="p-4">
                      <div className="flex items-center gap-2">
                        <span className="font-semibold text-gray-900 dark:text-gray-100">@{reel.author}</span>
                      </div>
                      <div className="text-xs text-gray-500 font-mono mt-1">ID: {reel.id}</div>
                    </td>
                    <td className="p-4">
                      <p className="text-sm text-gray-700 dark:text-gray-300 line-clamp-2">
                        {reel.caption || <span className="text-gray-400 italic">No caption</span>}
                      </p>
                    </td>
                ...
                <tr>
                  <td colSpan="5" className="p-16 text-center text-gray-500">
                    <Video size={48} className="mx-auto mb-4 text-gray-300 dark:text-gray-600" />
        ...
        <div className="px-6 py-4 border-t border-gray-200 dark:border-gray-700 flex items-center justify-between text-sm text-gray-500 bg-gray-50/30 dark:bg-gray-900/30">
          <span>Showing page <span className="font-semibold text-gray-900 dark:text-gray-100">{page}</span> of <span className="font-semibold text-gray-900 dark:text-gray-100">{totalPages}</span> ({totalReels} total reels)</span>
