...
const SharePostModal = ({ isOpen, onClose, postId }) => {
  ...
  const handleSend = async () => {
    ...
    setSending(true);
    try {
      await sharePost(postId, selectedUsers.map(u => u.id));
      onClose();
  ...
  return (
    <div onClick={onClose} className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <motion.div
